...
func main() {
    ...
    for _, q := range queries {
        ...
        query := fmt.Sprintf(".coafile created:" + q)
        ...
        for ; page <= maxPage; page++ {
            opts.Page = page
            result, response, err := client.Search.Code(ctx, query, opts)
            wait(response)
            ...
            for _,rs := range result.CodeResults {
                repos = append(repos, *rs.Repository)
            }

            maxPage = response.LastPage

            msg := fmt.Sprintf("page: %v/%v, size: %v, total: %v",
                page, maxPage, len(repos), *result.Total)
            log.Println(msg)
